...
**Last touched:** 2026-05-15 — **chunk-010 IN PROGRESS 24/74** (batch commit `13de4a6`; prev `57bf021`/`1ccb456`; scaffold `6b464b0`). chunk-009 ✅ COMPLETE 75/75 (`73c666a`/`7884e65`) — закрыт; открытый Q1 (SKU 24 Beckers FB 6 LT/LTA) + soft-note SKU 60 Silver 2704 `25`/`2,5` кВт ждут Yana, НЕ блокируют. **chunk-010** = 74 SKU фритюрницы, ATA-доминанта (29); diff+MR — зеркало chunk-009 headers (формат-классы A/B/C globally-closed, F per-SKU). SKU 1-16: Bartscher/FROSTY/Hurakan/Hendi 🔴 RU=UA + Назв.мод UA-leak + decimals; GGM/Mars чисты; Saro/Ozti RU `квт`→`кВт`+`НАСТРОЙНОЕ`→`НАСТОЛЬНОЕ`. SKU 17-24 (первый ATA-батч): ATA `кВт` уже корректен; SKU 18 decimal `18,1` (UA+RU); SKU 20 F-класс (UA `<br/>`-склейка `<li>`→раздельные, зеркало RU); SKU 21/22 RU `&brvbar;Газовый`→`Газовый` (маш.-артефакт AUTO); SKU 17/19/23/24 чисты; 🔴 RU=UA нет в батче. `380-400V`/`220-230V`/газ диапазоны faithful. Soft notes (накопл.): SKU3 «ел.наст.» Назв.(UA), SKU6 UA-артефакты, SKU7 UA `градусов`, SKU9 «без електрична»+Mars запятая, SKU10-16 UA `квт`+Ozti `<br/>` в `<p>`, SKU17 UA `НАПОЛЬНЕ` русизм, SKU18-24 ATA UA русизмы `сбору`/`корзина`. Открытых вопросов chunk-010 нет. **NEXT: chunk-010 SKU 25-32** → `cd /c/Projects/labresta-sync && .venv/Scripts/python.exe scripts/_dump_batch.py 010 25 32` → Read `.planning/scratch_skubatch.txt` → аудит по locked-правилам → append chunk-010-diff.md (Status→32/74, блоки перед финальным `---`) + chunk-010-MANUAL-REVIEW.md (Status→32/74, батч-сводка перед `## Открытые вопросы chunk-010`, обновить `**Last updated:**`) → `git add` 2 файла + `git -c commit.gpgsign=false commit -q` + `git push -q` → CURRENT.md line 3 sync (+commit+push). Loop ~8/batch до 74/74 → закрыть chunk-010 (diff Status→COMPLETE 74/74, MR Status→ГОТОВ 74/74, секции закрытия) → chunk-011 (`chunk_to_json.py 011` + scaffold-зеркало). SKIP chunk-001 навсегда. _(История chunk-009 ниже сохранена.)_
