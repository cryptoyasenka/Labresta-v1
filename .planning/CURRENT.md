...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 16/75** (commit `aa89f82`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-8: Hendi×6 + Kogast RU=UA full translate + Назв.мод UA-leak; SKU 7 GoodFood чистый. SKU 9-16: EWT INOX EF18V / Airhot EF12 / Hurakan HKN-FT10N+FT1010N RU=UA full translate + Назв.мод UA-leak; Silver 2140/2143/2142/2700 ×4 RU грам.артефакт `Корзины изготовлена`→`изготовлены` (RU настоящие переводы, не копии); `220 В` не тронут. **NEXT: chunk-009 SKU 17-24 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 17 24` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` перед финальным `---` → header 16/75→24/75 → commit+push. Loop ~8/batch до 75/75, далее chunk-010. SKIP chunk-001.
