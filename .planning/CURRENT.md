...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 32/75** (commit `760f30a`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-24 готовы (см. ниже). SKU 25-32: Beckers FR8/FROSTY CKEF-2 RU=UA full translate + Назв.мод UA-leak; Beckers FB4+4/FB6+6 модель-код RU `FB N LT`→`FB N LTA` (RU-заголовок усечён, UA+тело=LTA); GoodFood EF30V/EF20V RU маш.артефакт `ванная`→`ванна`; Beckers FB8+8 / FROSTY CKEF-1 чистые; `220 В` не тронут. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст FB-серии, не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 33-40 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 33 40` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` (diff: Status→40/75 + блоки перед финальным `---`; MR: батч-сводка перед `## Открытые вопросы`, новые Q перед `**Last updated:**`) → commit+push → CURRENT.md. Loop ~8/batch до 75/75, далее chunk-010 (`scripts/chunk_to_json.py 010` mirror header). SKIP chunk-001.
