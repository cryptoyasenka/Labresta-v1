...
**Last touched:** 2026-05-09 (Phase M apply ЗАБЛОКИРОВАН — обнаружен incomplete supplier feed)
**Status:** Phase M dry-run прод = 137 Path B флагов. **Аудит Apach + проверка реального XLSX = диагноз: фид от `np.com.ua/dealer-export?dealer_id=69781&filetype=xlsx&platform=horoshop` НЕПОЛНЫЙ**. Yana видит на дилерском портале товары которых нет в XLSX (APL ×8, APKE-77/PL, Стіл холодильний AFM 02/03/04). XLSX — 690 рядов, 158 Apach (парсер не дропает). Феед просто меньше чем витрина у поставщика. Phase M честно флагует, но флаги ложные для этих товаров. **Phase M apply НЕ выполнен**. Решение: разбираться с поставщиком/URL/scraping, потом повторить Phase M.
...
2. **Phase M apply — ЗАБЛОКИРОВАН до починки фида np.com.ua.** Yana глазами проверила (2026-05-09): AD46MV — реально нет у поставщика (truly missing); APL/APKE-77/AFM 02 — на дилерском портале ЕСТЬ, в нашем XLSX-экспорте НЕТ. Скачан real XLSX от feed_url напрямую → 690 рядов, 158 Apach, **0 совпадений** для APKE/APL/AD46MV/AD46DV. Парсер ничего не теряет (158=158). XLSX просто меньше каталога. Гипотезы: (a) `platform=horoshop` query-param фильтр, (b) `dealer_id=69781` неполный, (c) поставщик намеренно не экспортирует часть, (d) другой endpoint URL у np.com.ua. **Дальше: написать поставщику или попробовать URL-варианты (без `platform=horoshop`, другие filetype). До этого Phase M не применять — даст ~50%+ ложных сирот.**
