...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 40/75** (commit `45d51bd`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-32 готовы (см. ниже). SKU 33-40: Apach APFE-77P/PL + Tatra TEF.87 (зеркало sibling TEF.47) + Hurakan HKN-FDI8D/8T + FROSTY EFE-6 — 🔴 RU=UA full translate + Назв.мод UA-leak→Название (RU); GoodFood EF6/EF66 BLACK RU маш.артефакт `ванная`→`ванна` (RU-переводы корректны); Tatra TEF.47 decimal `10.5`→`10,5` (UA+RU); FROSTY EFE-6 decimal `4.50`→`4,50` (UA+RU); `220 В`/`380 В` не тронут. Soft notes: SKU 33 UA `Напруга: 220 Ст.` артефакт, SKU 34 UA `2,5 квт`/`60-200С` без °, SKU 40 Название (UA) без «електрична». **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст FB-серии, не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 41-48 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 41 48` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` (diff: Status→48/75 + блоки перед финальным `---`; MR: батч-сводка перед `## Открытые вопросы`, новые Q перед `**Last updated:**`) → commit+push → CURRENT.md. Loop ~8/batch до 75/75, далее chunk-010 (`scripts/chunk_to_json.py 010` mirror header). SKIP chunk-001.
