...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 48/75** (commit `710260e`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-40 готовы (см. ниже). SKU 41-48: FROSTY EFE-66/88 (зеркало sibling EFE-6) + Oztiryakiler OFEI8070 + Kogast EFT7/2x5 — 🔴 RU=UA full translate + Назв.мод UA-leak→Название (RU); Saro×4 (EF 6/PROFRI 4/6/44) RU операт. артефакты `квт`→`кВт`+`НАСТАЛЬНОЕ/НАСТРОЙНОЕ`→`НАСТОЛЬНОЕ` (RU-переводы корректны, рекуррентный Saro-template); decimal SKU41 `8.50`→`8,50` / SKU42 `2.5`→`2,5` / SKU47 `3.25`→`3,25` (UA+RU); `220 В`/`380V` не тронут. Soft notes: SKU 41 Название (UA) без «електрична», SKU 45-48 UA `квт` строчными. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст FB-серии, не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 49-56 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 49 56` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` (diff: Status→56/75 + блоки перед финальным `---`; MR: батч-сводка перед `## Открытые вопросы`, новые Q перед `**Last updated:**`) → commit+push → CURRENT.md. Loop ~8/batch до 75/75, далее chunk-010 (`scripts/chunk_to_json.py 010` mirror header). SKIP chunk-001.
