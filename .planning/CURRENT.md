...
**Last touched:** 2026-05-15 — **chunk-009 ✅ COMPLETE 75/75** (commit `73c666a`; diff Status=COMPLETE, MR Status=ГОТОВ, оба запушены). Открытый Q1 (SKU 24 Beckers FB 6 LT/LTA) + soft-note SKU 60 Silver 2704 `25`/`2,5` кВт ждут Yana (не блокируют). **NEXT: chunk-010** → `cd /c/Projects/labresta-sync && .venv/Scripts/python.exe scripts/chunk_to_json.py 010` → создать `.planning/translation-audit/chunks/chunk-010-diff.md` + `chunk-010-MANUAL-REVIEW.md` зеркалом chunk-009 headers (читать первые ~15 строк chunk-009-diff.md + первые ~14 строк chunk-009-MANUAL-REVIEW.md как шаблон; «009»→«010»; бренд-микс/кол-во SKU из вывода `_dump_batch.py` или chunk-010.json; формат-классы A/B/C globally-closed текст сохранить; F per-SKU). Затем `.venv/Scripts/python.exe scripts/_dump_batch.py 010 1 8` → Read scratch → аудит → diff+MR append → commit+push → CURRENT.md sync. Loop ~8/batch. SKIP chunk-001. _(Прежний прогресс chunk-009 ниже сохранён для истории.)_ A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-64 готовы (см. ниже). SKU 65-72: FROSTY EFS-11L/11L-2/12L/12L-2/132V/161V (EFS-шаблон класс 63/64) — 🔴 RU=UA full translate + Назв.мод UA-leak→Название (RU); Mars MF-3 decimal-only, Mars MF-5 без правок (RU-переводы корректны); decimal SKU65 `6.40` / SKU67 `9.30` / SKU68 `17.60` / SKU69 `15.30` / SKU71 `2.5` (UA+RU); SKU66 `12.00`/SKU70 `10.00` = формат-класс A (NN.00) → глоб. pass НЕ per-SKU; `220 В` не тронут. Soft notes: SKU 72 Назв.(UA) без «електрична» (класс 40/41/58), SKU 71/72 Mars RU лишняя запятая. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст, не авто-резолвлю, ждёт Yana). Soft-note на решение Yana: SKU 60 Silver 2704 расхождение `25` vs `2,5` кВт. **NEXT: chunk-009 ФИНАЛЬНЫЙ батч SKU 73-75 (3 SKU)** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 73 75` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md` (Status→**COMPLETE 75/75**, блоки перед финальным `---`) / `-MANUAL-REVIEW.md` (Status→**ГОТОВ 75/75**, батч-сводка перед `## Открытые вопросы`, финализировать Q1 формулировку, обновить `**Last updated:**` → «SKU 1-75 готовы, chunk-009 закрыт») → commit+push → CURRENT.md sync. **Затем chunk-010:** `.venv/Scripts/python.exe scripts/chunk_to_json.py 010` → создать `chunk-010-diff.md` + `chunk-010-MANUAL-REVIEW.md` зеркалом chunk-009 headers (locked-паттерны + формат-классы A/B/C globally closed + F per-SKU) → loop ~8 SKU/batch. SKIP chunk-001.
