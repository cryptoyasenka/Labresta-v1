...
**Last touched:** 2026-05-09 (Phase M apply ОТЛОЖЕН после прод-аудита Apach)
**Status:** Phase M dry-run на проде показал 137 Path B флагов; **детальный аудит Apach (33/137) выявил ~13 матчер-гэпов** (AHM250V250 vs AHM250V250C; AFM 02 vs AFM250VV200; AD46MV vs AD46M; AP5QD vs AP5QM; ASH05K (NEW) vs ASH05K R290) — это Phase N territory, не сироты. **Phase M apply НЕ выполнен** — рискованно до починки матчера или audit'а остальных 12 брендов. 677/677 tests green.
...
2. **Phase M apply — НЕ применять** до починки матчер-гэпов. Apach 33-флаг аудит на проде (2026-05-09): фид имеет 158 SP (60 in-stock + 98 out-of-stock — out-of-stock грузятся, фильтра нет), но catalog PPs имеют SKU-варианты которых нет в фиде точно. Серии где **феед НЕ имеет вообще** (truly missing, OK флагать): APL ×8, APACH без модели ×6, AMX ×2, ASM ×1, ACS ×1, ACG ×1 = ~19. Серии где **феед ИМЕЕТ ту же серию но другой суффикс** (matcher gap, НЕ сироты): AFM ×6, AD ×3, AP ×2, ASH ×1, AHM ×1 = ~13. Аналогичный аудит нужен для Hurakan/Gooder/Fagor/Silanos/Beckers/Airhot/Gastromix/Tatra/Cooleq/ITPIZZA/JAU/Scan (104 PPs). Yana — решить: (a) Phase N matcher fix первым, (b) audit per-brand и apply только truly-missing, (c) ручной триаж в UI.
