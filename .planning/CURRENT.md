...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 56/75** (commit `66c200a`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-48 готовы (см. ниже). SKU 49-56: Airhot EEF6 — 🔴 RU=UA full translate + Назв.мод UA-leak→Название (RU); Saro×5 (PROFRI 88 V/EF 4/EF 8/PROFRI 8V/EF 88) RU операт. артефакты `квт`→`кВт`+`НАСТАЛЬНОЕ/НАСТРОЙНОЕ`→`НАСТОЛЬНОЕ` (RU-переводы корректны, рекуррентный Saro-template); GoodFood EF12A2 RU маш. `ванная`→`ванна`; GoodFood EF12A1 без правок (RU корректен); decimal SKU52 `3.25`→`3,25` / SKU56 `2.5`/`3.6`/`4.5` (UA+RU); `220 В` не тронут. Soft notes: SKU 49-53 UA `квт` строчными, SKU 49 UA `<br/>`-склейка `<p>` (не F-класс), SKU 51/53 UA `Тени`-артефакт, SKU 54/55 `60-200 С` без градуса. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст FB-серии, не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 57-64 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 57 64` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` (diff: Status→64/75 + блоки перед финальным `---`; MR: батч-сводка перед `## Открытые вопросы`, новые Q перед `**Last updated:**`) → commit+push → CURRENT.md. Loop ~8/batch до 75/75, далее chunk-010 (`scripts/chunk_to_json.py 010` mirror header). SKIP chunk-001.
