...
**Last touched:** 2026-05-15 — **chunk-010 IN PROGRESS 16/74** (batch commit `57bf021`; prev `1ccb456`; scaffold `6b464b0`). chunk-009 ✅ COMPLETE 75/75 (`73c666a`/`7884e65`) — закрыт; открытый Q1 (SKU 24 Beckers FB 6 LT/LTA) + soft-note SKU 60 Silver 2704 `25`/`2,5` кВт ждут Yana, НЕ блокируют. **chunk-010** = 74 SKU фритюрницы, ATA-доминанта (29); diff+MR — зеркало chunk-009 headers (формат-классы A/B/C globally-closed, F per-SKU). SKU 1-8: Bartscher/FROSTY/Hurakan/Hendi 🔴 RU=UA full translate + Назв.мод UA-leak→Название(RU) + decimals; GGM SKU7/8 чисты. SKU 9-16: Mars MF-3+3 (SKU9) чист; Saro PROFRI 66 (SKU10) decimal `6,5`; Saro/Ozti template SKU 10-16 RU `квт`→`кВт` + `НАСТРОЙНОЕ`→`НАСТОЛЬНОЕ` (AUTO маш.-артефакт); Назв.мод RU корректны, 🔴 RU=UA нет в батче. `220`/`220 В`/`380 В`/газ не тронуты. Soft notes: SKU3 «ел.наст.» в Назв.(UA), SKU6 UA-артефакты, SKU7 UA `градусов`, SKU9 «без електрична»+Mars RU запятая, SKU10-16 UA `квт` строчными + Ozti `<br/>` в `<p>` faithful. Открытых вопросов chunk-010 нет. **NEXT: chunk-010 SKU 17-24** → `cd /c/Projects/labresta-sync && .venv/Scripts/python.exe scripts/_dump_batch.py 010 17 24` → Read `.planning/scratch_skubatch.txt` → аудит по locked-правилам → append chunk-010-diff.md (Status→24/74, блоки перед финальным `---`) + chunk-010-MANUAL-REVIEW.md (Status→24/74, батч-сводка перед `## Открытые вопросы chunk-010`, обновить `**Last updated:**`) → `git add` 2 файла + `git -c commit.gpgsign=false commit -q` + `git push -q` → CURRENT.md line 3 sync (+commit+push). Loop ~8/batch до 74/74 → закрыть chunk-010 (diff Status→COMPLETE 74/74, MR Status→ГОТОВ 74/74, секции закрытия) → chunk-011 (`chunk_to_json.py 011` + scaffold-зеркало). SKIP chunk-001 навсегда. _(История chunk-009 ниже сохранена.)_
