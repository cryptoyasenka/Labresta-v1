...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 64/75** (commit `1e77538`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-56 готовы (см. ниже). SKU 57-64: Airhot EEF6+6 (sibling 56) / Beckers FR 8+8 LT / Sirman ELDORADO 5 / FROSTY EFS-4L-2 / EFS-6L-2 (sibling 63) — 🔴 RU=UA full translate + Назв.мод UA-leak→Название (RU); Silver 2705/2704 decimal-only (RU-переводы корректны); Silver 2141 без правок (RU корректен); decimal SKU57 `5.3`/`6.4` / SKU59 `12.8` / SKU60 `6.2` / SKU63 `8.40` / SKU64 `8.70` (UA+RU); `220 В` не тронут; модель-код Beckers FR 8+8 LT единогласен (не LT/LTA-конфликт). Soft notes: SKU 60 расхождение `25` vs `2,5` кВт в источнике (на решение Yana), SKU 58 UA `(фритюрі)..`, SKU 59/60 RU `ванночка` диминутив, SKU 61 UA `Кошики виготовлена`/`Удобная`. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA — генуинный заголовок↔тело конфликт 4:4 + family-контекст FB-серии, не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 65-72 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 65 72` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` (diff: Status→72/75 + блоки перед финальным `---`; MR: батч-сводка перед `## Открытые вопросы`, новые Q перед `**Last updated:**`) → commit+push → CURRENT.md. Loop ~8/batch до 75/75 (последний батч SKU 73-75 = 3 SKU), далее chunk-010 (`scripts/chunk_to_json.py 010` mirror header). SKIP chunk-001.
