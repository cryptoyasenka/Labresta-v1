...
**Last touched:** 2026-05-15 — **chunk-010 IN PROGRESS 8/74** (batch commit `1ccb456`; scaffold `6b464b0`). chunk-009 ✅ COMPLETE 75/75 (`73c666a`/`7884e65`) — закрыт; открытый Q1 (SKU 24 Beckers FB 6 LT/LTA) + soft-note SKU 60 Silver 2704 `25`/`2,5` кВт ждут Yana, НЕ блокируют. **chunk-010** = 74 SKU фритюрницы, ATA-доминанта (29); diff+MR — зеркало chunk-009 headers (формат-классы A/B/C globally-closed, F per-SKU). SKU 1-8: Bartscher A162810E/A162410E, FROSTY EFS-162V, Hurakan FT88N/FT8N, Hendi 205815 — 🔴 RU=UA full translate + Назв.мод UA-leak→Название(RU); decimal SKU1 `6.7`/`3.25`, SKU2 `19.50`, SKU3 `5.1`/`2.0` (UA+RU); GGM EFK653-8+8 (SKU7)/EFK644-12+12 (SKU8) чисты (RU корректен). `220V`/`220 В`/`380 В` не тронуты. Soft notes: SKU3 дескриптор «ел.наст.» в Назв.(UA) нет в Назв.(RU), SKU6 UA-артефакты (`пирожов`/`Демонтируемая`/`Виймаємо`), SKU7 UA `градусов`. Открытых вопросов chunk-010 нет. **NEXT: chunk-010 SKU 9-16** → `cd /c/Projects/labresta-sync && .venv/Scripts/python.exe scripts/_dump_batch.py 010 9 16` → Read `.planning/scratch_skubatch.txt` → аудит по locked-правилам → append chunk-010-diff.md (Status→16/74, блоки перед финальным `---`) + chunk-010-MANUAL-REVIEW.md (Status→16/74, батч-сводка перед `## Открытые вопросы chunk-010`, обновить `**Last updated:**`) → `git add` 2 файла + `git -c commit.gpgsign=false commit -q` + `git push -q` → CURRENT.md line 3 sync (+commit+push). Loop ~8/batch до 74/74 → закрыть chunk-010 (diff Status→COMPLETE 74/74, MR Status→ГОТОВ 74/74, секции закрытия) → chunk-011 (`chunk_to_json.py 011` + scaffold-зеркало). SKIP chunk-001 навсегда. _(История chunk-009 ниже сохранена.)_
