...
**Last touched:** 2026-05-15 — **chunk-009 IN PROGRESS 24/75** (commit `8ee03a4`). A/B/C → global assembly-pass (НЕ per-SKU diff/FLAG; F per-SKU при встрече). SKU 1-16 готовы (см. ниже). SKU 17-24: Hurakan HKN-FT66N+FT6N / Airhot EF12+12 / Beckers FB6 RU=UA full translate + Назв.мод UA-leak; Silver 2702/2703 ×2 RU грам.артефакт `Корзины изготовлена`→`изготовлены` (RU настоящие переводы); Beckers FB4 LTA модель-код RU `FB 4 LT`→`FB 4 LTA` (6:2 доминанта источника); Sirman Klondike 8R чистый; `220 В` не тронут. **Открытые вопросы chunk-009: Q1** (SKU 24 Beckers FB 6 модель-код LT vs LTA, 4:4 split — не авто-резолвлю, ждёт Yana). **NEXT: chunk-009 SKU 25-32 batch** → `.venv/Scripts/python.exe scripts/_dump_batch.py 009 25 32` → Read `.planning/scratch_skubatch.txt` → аудит → append `chunk-009-diff.md`/`-MANUAL-REVIEW.md` перед финальным `---` (Q-вопросы в MANUAL-REVIEW перед строкой `**Last updated:**`) → header 24/75→32/75 → commit+push → CURRENT.md. Loop ~8/batch до 75/75, далее chunk-010. SKIP chunk-001.
