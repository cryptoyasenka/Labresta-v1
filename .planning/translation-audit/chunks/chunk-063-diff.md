...
**Status:** b11 DONE 88/88 — chunk-063 ЗАКРЫТ (b1-b11 по 8 SKU)
...
## b11 (SKU 81-88, rows 82-89) — chunk-063 ЗАКРЫТ 88/88

Изменения только в `chunk-063-fixed.xlsx` (gitignored). Apply key `Артикул` (col1) НЕ менялся.

**blk триплет (3): col5 ← col7 (genuine RU); col36 ← faithful RU (skeleton == UA col35)**

- **SKU 83 r84 ART 2434365116 — AIRHOT SB-5700S мармит для супа**
  - col5: `Марміт для супу АIRHOT SB-5700S` → `Мармит для супа АIRHOT SB-5700S`
  - col36: UA-leak (==col35) → RU `<p>…</p> <p><strong>…</strong></p> <ul>\n` 11×`<li>` `\n</ul>` (`&mdash;` verbatim, `285x285x352`/`300x300x395` лат.x; UA-апостроф «об'єм»→«объем»)
- **SKU 84 r85 ART 2434373120 — AIRHOT SB-6000S мармит для супа**
  - col5: `Марміт для супу АIRHOT SB-6000S` → `Мармит для супа АIRHOT SB-6000S`
  - col36: UA-leak → RU `<p>…</p> <p><strong>…</strong></p> <ul>\n` 13×`<li>` `\n</ul>` (`&mdash;` verbatim, `385x385x366`/`340x340x395`; UA `Країн-виробник`→`Страна-производитель`)
- **SKU 88 r89 ART 2567549749 — Bartscher 700355 витрина нейтральная**
  - col5: `Вітрина нейтральна Bartscher 700355` → `Витрина нейтральная Bartscher 700355`
  - col36: UA-leak → RU `<p>…</p> <p><strong>…</strong></p> <ul>\n` 7×`<li>` `\n</ul> <p>…</p> <ul>\n` 4×`<li>` `\n</ul>` (`700355`/`2`/`13.2`/`360`/`600`/`515`/`15.5`/`627`/`389`/`546`)

**blknotrip (3): имя lang-neutral c5==c7 — col5 НЕ тронут; col36 ← faithful RU (skeleton == UA col35)**

- **SKU 85 r86 ART 2464192575 — Bartscher 150992 термос** — c5==c7 `Термос Bartscher 150992` (НЕ тронут); col36 UA-leak → RU `<p>…</p> <p><strong>…</strong></p> <ul>\n` 13×`<li>` `\n</ul> <p>…</p> <ul>\n` 4×`<li>` `\n</ul>` (`3`/`150`/`295`/`1.6`/`170`/`170`/`375`/`1.8`/`180`/`180`/`390`)
- **SKU 86 r87 ART 2464194869 — Bartscher 150993 термос** — c5==c7 `Термос Bartscher 150993` (НЕ тронут); col36 → RU 11+4×`<li>` (`5`/`370`/`150`/`2.4`/`181`/`245`/`428`/`2.65`/`220`/`220`/`445`; UA `Німеччина`→`Германия`)
- **SKU 87 r88 ART 2565697144 — Bartscher 190117 термос** — c5==c7 `Термос Bartscher 190117` (НЕ тронут); col36 → RU 10+4×`<li>` (`190117`/`1.9`/`150`/`1.3`/`150`/`200`/`335`/`1.5`/`180`/`180`/`335`)

**blknochg (2): c5==c7 genuine RU, c35≠c36 (RU desc уже переведён) — fixed НЕ тронут, soft-notes pre-existing**

- **SKU 81 r82 ART 2331821398 — SARO SKZ-12 электросупница** — c5==c7 `Электросупница SARO SKZ-12`; c36 уже RU. Soft-note: `Подключение, в 220 В` (UA `Підключення, в 220`); `50&deg;С до 98&deg;С.` vs UA `50 &deg; С до 98 &deg; С`; первый заголовок без `<strong>`. Не правим.
- **SKU 82 r83 ART 2331843799 — SARO BMH 210 мармит** — c5==c7 `Мармит SARO BMH 210`; c36 уже RU. Soft-note: первый заголовок `<p>Технические характеристики:</p>` без `<strong>` (UA `<strong>`). Не правим.

**SKIP-НП (0).**

**Verify:** 161 PASS / 0 FAIL.

**chunk-063 ЗАКРЫТ: 88/88 (b1..b11). Открытых вопросов 0.**
