...
## SKU 65/79 — Печь для пиццы Restoitalia RESTO 4 (380) (Артикул 420822918) — 🔴 RU=UA + Назв.мод (RU) UA-leak

**Поле:** Название модификации (RU)
**Было:** `Піч для піци Restoitalia RESTO 4 (380)`
**Стало:** `Печь для пиццы Restoitalia RESTO 4 (380)`
...
```
<p>Печь для пиццы, имеет одну камеру размером 660х660х140 мм, вместимость — 4 пиццы диаметром 30 см или 4 пиццы диаметром 33 см, дно камеры имеет огнеупорный камень. Облицовка полностью из нержавеющей стали, боковые части и задняя часть изготовлены из высококачественного крашеного металла, внутри камера изготовлена из алюминизированной стали. Механическая панель управления, подсветка, жаропрочное смотровое окно, температурный диапазон от 50 до 500 градусов, 2 нагревательных элемента для каждой камеры.</p> <ul> <li>Напряжение 380 В</li> <li>Мощность 4,7 кВт</li> <li>Габариты: Ш 805 х Г 925 х В 410 мм</li> </ul>
```

*(blknotrip — `desc UA==RU` **True** 🔴 (RU = полная UA копия); `nm_ua`==`nm_ru` `Піч для піци Restoitalia RESTO 4 (380)` (UA-leak в RU nm); `nazv_ru` genuine `Печь для пиццы Restoitalia RESTO 4 (380)` → AUTO Назв.мод (RU)=`nazv_ru`. **RESTO ITALIA NEW brand-family в проекте** Italian pizza-oven brand (Restoitalia). RESTO ITALIA НЕ ∈ NP-SET → обычная обработка. RESTO 4 = пицца-печь 1-камера 660×660×140 мм / 4 пиццы Ø30 см или Ø33 см / огнеупорный камень / 380 В / 4,7 кВт / Ш 805 × Г 925 × В 410 мм / нержавеющая сталь / алюминизированная сталь внутри / 2 ТЭНа на камеру. AUTHORED RU 1:1 mirror UA structure 1 `<p>` + 1 `<ul>/3li`. SOFT применено: (1) `Піч для піци, має одну камеру розміром` → `Печь для пиццы, имеет одну камеру размером`; (2) `місткість — 4 піци діаметром 30 см або 4 піци діаметром 33 см` → `вместимость — 4 пиццы диаметром 30 см или 4 пиццы диаметром 33 см`; (3) `дно камери має вогнетривкий камінь` → `дно камеры имеет огнеупорный камень`; (4) `Облицювання повністю з неіржавкої сталі` → `Облицовка полностью из нержавеющей стали`; (5) `бічні частини та задня частина виготовлені з високоякісного фарбованого металу` → `боковые части и задняя часть изготовлены из высококачественного крашеного металла`; (6) `всередині камера виготовлена з алюмінізованої сталі` → `внутри камера изготовлена из алюминизированной стали`; (7) `Механічна панель керування, підсвічування, жароміцне оглядове вікно` → `Механическая панель управления, подсветка, жаропрочное смотровое окно`; (8) `температурний діапазон від 50 до 500 градусів` → `температурный диапазон от 50 до 500 градусов` (UA уже без HTML entity, plain `градусів` → `градусов` — POL B/Rule B N/A здесь); (9) `2 нагрівальні елементи для кожної камери` → `2 нагревательных элемента для каждой камеры`; (10) `Напруга` → `Напряжение`; (11) `Потужність` → `Мощность`; (12) `Габарити: Ш 805 х Г 925 х В 410 мм` → `Габариты: Ш 805 х Г 925 х В 410 мм` Cyr х axis-prefix labels Ш/Г/В LIVE preserve. **POL1/POL2/POL3/POL4 НЕ trigger** (RESTO ITALIA НЕ ∈ NP-SET, нет U+02DA, нет closed-set typos, oven-domain). **Rule B (glyph-normalize) N/A** (UA уже plain `градусів` без HTML entity). UA-cell mod b9 += 0. RESTO ITALIA Restoitalia RESTO 4 NEW brand-family Italian pizza-oven. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=420822918)*

---

## SKU 66/79 — Печь для пиццы Restoitalia RESTO 44 (380) NC (Артикул 420822920) — 🔴 RU=UA + Назв.мод (RU) UA-leak

**Поле:** Название модификации (RU)
**Было:** `Піч для піци Restoitalia RESTO 44 (380) NC`
**Стало:** `Печь для пиццы Restoitalia RESTO 44 (380) NC`

**Поле:** Описание товара (RU)
**Было:** (полностью идентично UA — украинский текст)
**Стало:** (полный перевод RU тег-в-тег — sister SKU 65 с 2-chamber adjust):

```
<p>Печь для пиццы, имеет две камеры размером по 660х660х140 мм каждая, 8 пицц диаметром 30 см или 8 пицц диаметром 33 см. Дно камеры имеет огнеупорный камень. Облицовка полностью из нержавеющей стали, боковые части и задняя часть изготовлены из высококачественного крашеного металла, внутри камера изготовлена из алюминизированной стали. Механическая панель управления, подсветка, жаропрочное смотровое окно, температурный диапазон от 50 до 500 градусов, 2 нагревательных элемента для каждой камеры.</p> <ul> <li>Напряжение 380 В</li> <li>Мощность 9,4 кВт</li> <li>Габариты: Ш 805 х Г 925 х В 710 мм</li> </ul>
```

*(blknotrip — `desc UA==RU` **True** 🔴; `nm_ua`==`nm_ru` UA-leak; `nazv_ru` genuine `Печь для пиццы Restoitalia RESTO 44 (380) NC` → AUTO. **Sister SKU 65** RESTO 44 NC = 2-chamber version of RESTO 4 — различия только chamber-count 2 vs 1 + capacity 8 pizz vs 4 + Мощность 9,4 кВт vs 4,7 + В 710 vs 410 мм; rest identical. RESTO ITALIA NEW brand-family continuation. AUTHORED RU 1:1 mirror UA structure 1 `<p>` + 1 `<ul>/3li`. SOFT применено: identical SKU 65 set (12 терминов) + (a) `дві камери розміром по` → `две камеры размером по`; (b) `8 піц діаметром 30 см або 8 піц діаметром 33 см` → `8 пицц диаметром 30 см или 8 пицц диаметром 33 см`; (c) `Дно камери` → `Дно камеры`; (d) `підсвітка` → `подсветка` (вместо `підсвічування` в SKU 65 — supplier intra-brand inconsistency — обе формы → `подсветка`); (e) `Мощность 9,4 кВт`; (f) `В 710 мм`. **POL1/POL2/POL3/POL4 НЕ trigger**. **Rule B N/A** (UA plain `градусів`). UA-cell mod b9 += 0. RESTO ITALIA RESTO 44 NC sister SKU 65 NEW brand-family. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=420822920)*

---

## SKU 67/79 — Печь для пиццы GGF ES 4+4 (Артикул 478693159) — blknochg pure + Rule B RU glyph-normalize
...
**Поле:** Описание товара (RU)
**Было:** (RU имеет 🚨 HTML entity `&deg;` + Cyr `С` U+0421 в температурном диапазоне):
```
<p>Печь для пиццы имеет две секции и корпус из нержавеющей стали.</p> <p>Технические характеристики:</p> <ul> <li>натуральный под</li> <li>температурный режим от 50&deg;С до 500&deg;С</li> <li>размер камеры 660х660х140 мм.</li> <li>габариты 900х785х750 мм</li> <li>мощность: 8,4 кВт.</li> <li>Напряжение: 220/380 В.</li> <li>Вес: 123 кг</li> </ul>
```
**Стало:** (substring replace `от 50&deg;С до 500&deg;С` → `от 50 °C до 500 °C` — entity → Unicode `°`, Cyr `С` → Lat `C`, добавлены пробелы согласно формату `50 °C`):
```
<p>Печь для пиццы имеет две секции и корпус из нержавеющей стали.</p> <p>Технические характеристики:</p> <ul> <li>натуральный под</li> <li>температурный режим от 50 °C до 500 °C</li> <li>размер камеры 660х660х140 мм.</li> <li>габариты 900х785х750 мм</li> <li>мощность: 8,4 кВт.</li> <li>Напряжение: 220/380 В.</li> <li>Вес: 123 кг</li> </ul>
```

*(blknochg pure + **Rule B glyph-normalize применён ТОЛЬКО к RU**. `desc UA!=RU` (UA уже clean: `від 50 °C до 500 °C` с Unicode `°` + Lat `C` + пробел — supplier для UA выбрал correct format; supplier для RU выбрал HTML-entity + Cyr `С` без пробела — cross-locale supplier-side translate-time inconsistency); `nm_ua`!=`nm_ru` `Піч для піци GGF ES 4+4` vs `Печь для пиццы GGF ES 4+4` correctly translated; `nm_ru`==`nazv_ru` clean. **GGF continuation** (b8 SKU 58 GGF ES 4 первый GGF в проекте, b9 SKU 67/68/69 — GGF расширение до 4 SKU). GGF НЕ ∈ NP-SET → обычная обработка. **Rule B (Yana 2026-05-21 global glyph-normalize)**: HTML entity `&deg;` → Unicode `°` (U+00B0); Cyr `С` (U+0421) после градуса → Lat `C` (U+0043); формат `50 °C` (число + пробел + `°` + `C`). Применяется forward-only. Subj substring `от 50&deg;С до 500&deg;С` → `от 50 °C до 500 °C` — единичный change в RU desc, остальная часть body unchanged (genuine RU перевод supplier-side OK кроме glyph mess). UA desc unchanged (UA уже clean). **POL2 НЕ trigger** (нет U+02DA, `&deg;` HTML entity ≠ U+02DA degree-glyph). **POL1/POL4 НЕ trigger** (RU clean без closed-set typos). **POL3 N/A** (GGF НЕ ∈ NP-SET). UA-cell mod b9 += 0 (UA уже clean). RU-cell mod b9 += 1 (glyph-normalize). GGF ES 4+4 — Rule B FIRST APPLICATION в проекте — устанавливает forward standard формата `50 °C`. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=478693159)*

---

## SKU 68/79 — Печь для пиццы GGF Micro V (Артикул 478702668) — blknochg pure + Rule B UA+RU glyph-normalize
...
**Поле:** Описание товара (UA)
**Было:** (UA содержит 🚨 HTML entity `&deg;` в температурном диапазоне):
`<li>температурний режим від +50 &deg;C до +500 &deg;C</li>`
**Стало:** (substring replace `+50 &deg;C до +500 &deg;C` → `+50 °C до +500 °C` — entity → Unicode `°`, формат preserved):
`<li>температурний режим від +50 °C до +500 °C</li>`

**Поле:** Описание товара (RU)
**Было:** (RU 🚨 asymmetric — `°C` только после `+500`, нет после `+50`; + HTML entity):
`<li>температурный режим от +50 до +500 &deg;C</li>`
**Стало:** (substring replace `от +50 до +500 &deg;C` → `от +50 °C до +500 °C` — symmetric расстановка + entity → Unicode `°`):
`<li>температурный режим от +50 °C до +500 °C</li>`

*(blknochg pure + **Rule B glyph-normalize применён к UA+RU обеим**. `desc UA!=RU` (genuine отдельный RU перевод, но оба cells содержали glyph mess); `nm_ua`!=`nm_ru` `Піч для піци GGF Micro V` vs `Печь для пиццы GGF Micro V` correctly translated; `nm_ru`==`nazv_ru` clean. **GGF continuation** (3rd GGF SKU b9). Micro V = compact professional electric pizza-oven 1 камера 405×405×110 мм / 1 пицца Ø40 см / 220 В / 2,2 кВт / 50-500°C / 26 кг / mineral wool insulation. **Rule B applied dual-cell**: (UA) `+50 &deg;C до +500 &deg;C` → `+50 °C до +500 °C` (одна `&deg;` сохраняла пробел перед entity — пробел тоже dropped в favour формата `°C`); (RU) `+50 до +500 &deg;C` → `+50 °C до +500 °C` (asymmetric → symmetric расстановка, добавлено `°C` после `+50` для clarity, entity → Unicode). UA-cell mod b9 += 1 (Rule B). RU-cell mod b9 += 1. Остальная часть UA + RU body unchanged. **POL2 НЕ trigger**. **POL1/POL4 НЕ trigger**. **POL3 N/A** (GGF НЕ ∈ NP-SET). GGF Micro V — Rule B применён dual-cell, supplier-side glyph inconsistency исправлен. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=478702668)*

---

## SKU 69/79 — Печь для пиццы GGF Micro 2V (Артикул 478706872) — blknochg pure + Rule B UA+RU glyph-normalize (sister SKU 68)
...
**Поле:** Описание товара (UA)
**Было:** `<li>температурний режим від +50 &deg;C до +500 &deg;C</li>`
**Стало:** `<li>температурний режим від +50 °C до +500 °C</li>`

**Поле:** Описание товара (RU)
**Было:** `<li>температурный режим от +50 до +500 &deg;C</li>`
**Стало:** `<li>температурный режим от +50 °C до +500 °C</li>`

*(blknochg pure + **Rule B glyph-normalize sister SKU 68 same pattern**. `desc UA!=RU` (genuine RU перевод); `nm_ua`!=`nm_ru` correctly translated; `nm_ru`==`nazv_ru` clean. **GGF continuation** (4th GGF SKU b9). **Sister SKU 68** Micro 2V = 2-section version of Micro V (1V) — различия только section-count 2 vs 1 + capacity 1 pizza в каждую секцию + dimensions 555×460×535 vs 555×460×290 + мощность 4,4 кВт vs 2,2 + напряжение 380 В vs 220 + вес 46 vs 26 кг; rest identical structure. Rule B applied identically dual-cell. UA-cell mod b9 += 1. RU-cell mod b9 += 1. **POL2 НЕ trigger**. **POL1/POL4 НЕ trigger**. **POL3 N/A**. GGF Micro 2V sister SKU 68 — Rule B sister application. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=478706872)*

---

## SKU 70/79 — Печь для пиццы ITPIZZA MS1 (Артикул 490788754) — 🔴 RU=UA + Назв.мод (RU) UA-leak

**Поле:** Название модификации (RU)
**Было:** `Піч для піци ITPIZZA MS1`
**Стало:** `Печь для пиццы ITPIZZA MS1`

**Поле:** Описание товара (RU)
**Было:** (полностью идентично UA — украинский текст с `°C` уже clean)
**Стало:** (полный перевод RU тег-в-тег с `°C` preserve):

```
<p>Печь для пиццы имеет одну секцию на 1 пиццу. Фронтальная панель изготовлена из высококачественной нержавеющей стали. Боковые и задняя панели изготовлены из крашеной стали. Жаропрочное стекло дверец камеры выпечки. Внутренняя подсветка камеры выпечки. Поверхность для выпекания изготовлена из огнеупорного материала (шамот).</p> <p>Технические характеристики:</p> <ul> <li>натуральный под</li> <li>температурный режим от 50 °C до 320 °C</li> <li>размер камеры внутренний: 410x360x90 мм.</li> <li>габариты 568x430x280 мм</li> <li>мощность: 1,7 кВт.</li> <li>напряжение: 220 В.</li> <li>вес: 18 кг</li> </ul>
```

*(blknotrip — `desc UA==RU` **True** 🔴 (RU = UA копия); `nm_ua`==`nm_ru` UA-leak; `nazv_ru` genuine `Печь для пиццы ITPIZZA MS1` → AUTO. **ITPIZZA continuation** (prior chunks had ITPIZZA ML66L blk триплет — b9 расширение ITPIZZA до MS-серия). ITPIZZA НЕ ∈ NP-SET → обычная обработка. MS1 = пицца-печь 1-секция / 1 пицца / 410×360×90 мм / 220 В / 1,7 кВт / 50-320°C / 18 кг / шамот / 568×430×280. AUTHORED RU 1:1 mirror UA structure 1 `<p>` + 1 `<p>Технічні</p>` + 1 `<ul>/7li`. SOFT применено: (1) `Піч для піци має одну секцію на 1 піцу` → `Печь для пиццы имеет одну секцию на 1 пиццу`; (2) `Фронтальна панель виготовлена з високоякісної неіржавкої сталі` → `Фронтальная панель изготовлена из высококачественной нержавеющей стали`; (3) `Бічні та задня панелі виготовлені з пофарбованої сталі` → `Боковые и задняя панели изготовлены из крашеной стали`; (4) `Жароміцне скло дверцят камери випічки` → `Жаропрочное стекло дверец камеры выпечки`; (5) `Внутрішня підсвітка камери випічки` → `Внутренняя подсветка камеры выпечки`; (6) `Поверхня для випікання виготовлена з вогнетривкого матеріалу (шамот)` → `Поверхность для выпекания изготовлена из огнеупорного материала (шамот)`; (7) `Технічні характеристики` → `Технические характеристики`; (8) `натуральний під` → `натуральный под`; (9) `температурний режим від 50 °C до 320 °C` → `температурный режим от 50 °C до 320 °C` (UA уже clean `°C` — Rule B N/A); (10) `розмір камери внутрішній` → `размер камеры внутренний`; (11) `габарити` → `габариты`; (12) `потужність` → `мощность`; (13) `напруга` → `напряжение`; (14) `вага` → `вес`. **POL1/POL2/POL3/POL4 НЕ trigger** (ITPIZZA НЕ ∈ NP-SET, UA cells clean, нет U+02DA, нет closed-set typos). **Rule B N/A** (UA уже clean `°C`). UA-cell mod b9 += 0. ITPIZZA MS1 NEW MS-серия model line в ITPIZZA family (после ML66L). META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=490788754)*

---

## SKU 71/79 — Печь для пиццы ITPIZZA MS1+1 (Артикул 490788755) — 🔴 RU=UA + Назв.мод (RU) UA-leak (sister SKU 70)

**Поле:** Название модификации (RU)
**Было:** `Піч для піци ITPIZZA MS1+1`
**Стало:** `Печь для пиццы ITPIZZA MS1+1`

**Поле:** Описание товара (RU)
**Было:** (полностью идентично UA — украинский текст)
**Стало:** (полный перевод RU тег-в-тег — sister SKU 70 с 2-section adjust):

```
<p>Печь для пиццы имеет две секции, производительность: 2 пиццы (1 пицца в каждую секцию). Фронтальная панель изготовлена из высококачественной нержавеющей стали. Боковые и задняя панели изготовлены из крашеной стали. Жаропрочное стекло дверец камеры выпечки. Внутренняя подсветка камеры выпечки. Поверхность для выпекания изготовлена из огнеупорного материала (шамот)</p> <p>Технические характеристики:</p> <ul> <li>натуральный под</li> <li>температурный режим от 50 °C до 320 °C</li> <li>размер камеры внутренний: 410x360x90 мм.</li> <li>габариты: 568x430x425 мм</li> <li>мощность: 2,4 кВт.</li> <li>Напряжение: 220 В.</li> <li>Вес: 32 кг</li> </ul>
```

*(blknotrip — `desc UA==RU` **True** 🔴; `nm_ua`==`nm_ru` UA-leak; `nazv_ru` genuine → AUTO. **Sister SKU 70** MS1+1 = 2-section version of MS1 — различия только section-count 2 vs 1 + capacity 2 pizzas (1 per section) + мощность 2,4 кВт vs 1,7 + В425 vs В280 мм + вес 32 vs 18 кг. ITPIZZA continuation. AUTHORED RU mirror SKU 70 + (a) `має дві секції, продуктивність: 2 піци (1 піца в кожну секцію)` → `имеет две секции, производительность: 2 пиццы (1 пицца в каждую секцию)`; (b) `габарити: 568x430x425 мм`; (c) `потужність: 2,4 кВт`; (d) `Напруга: 220 В` capitalized vs SKU 70 lowercase `напруга` — supplier intra-серия capitalization inconsistency LIVE preserve mirror в RU `Напряжение`; (e) `Вага: 32 кг` capitalized. **POL1/POL2/POL3/POL4 НЕ trigger**. **Rule B N/A** (UA clean `°C`). UA-cell mod b9 += 0. ITPIZZA MS1+1 sister SKU 70. META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=490788755)*

---

## SKU 72/79 — Печь для пиццы ITPIZZA MS4 (Артикул 490788756) — 🔴 RU=UA + Назв.мод (RU) UA-leak + UA Rule A (`32 див`→`32 см`) + UA Rule B glyph-normalize

**Поле:** Название модификации (RU)
**Было:** `Піч для піци ITPIZZA MS4`
**Стало:** `Печь для пиццы ITPIZZA MS4`

**Поле:** Описание товара (UA)
**Было:** (UA содержит 2 🚨: (a) typo `32 див` — должно быть `32 см`; (b) HTML entity `&deg;` + Cyr `С` U+0421 в temp range):
```
<p>Піч для піци має одну секцію на 4 піци діаметром до 32 див. Фронтальна панель виготовлена з високоякісної нержавійючої сталі. Бічні і задня панелі виготовлені з пофарбованої сталі. Жароміцне скло дверцят камери випічки. Внутрішня підсвітка камери випічки. Поверхня для випічки виготовлена з вогнетривкого матеріалу (шамот)</p> <p>Технічні характеристики:</p> <ul>
<li>натуральний під</li>
<li>температурний режим від 50&deg;С до 500&deg;С</li>
<li>внутрішній розмір камери: 660х660х140 мм.</li>
<li>габарити: 975x924x413 мм</li>
<li>потужність: 4,7 кВт.</li>
<li>Напруга: 380 В.</li>
<li>Вага: 90 кг</li>
</ul>
```
**Стало:** (2 substring replace: (a) Rule A `32 див` → `32 см`; (b) Rule B `50&deg;С до 500&deg;С` → `50 °C до 500 °C`):
```
<p>Піч для піци має одну секцію на 4 піци діаметром до 32 см. Фронтальна панель виготовлена з високоякісної нержавійючої сталі. Бічні і задня панелі виготовлені з пофарбованої сталі. Жароміцне скло дверцят камери випічки. Внутрішня підсвітка камери випічки. Поверхня для випічки виготовлена з вогнетривкого матеріалу (шамот)</p> <p>Технічні характеристики:</p> <ul>
<li>натуральний під</li>
<li>температурний режим від 50 °C до 500 °C</li>
<li>внутрішній розмір камери: 660х660х140 мм.</li>
<li>габарити: 975x924x413 мм</li>
...
**Поле:** Описание товара (RU)
**Было:** (полностью идентично UA — украинский текст с 2 ошибками)
**Стало:** (полный AUTHORED перевод RU с normalized `°C` формат):

```
<p>Печь для пиццы имеет одну секцию на 4 пиццы диаметром до 32 см. Фронтальная панель изготовлена из высококачественной нержавеющей стали. Боковые и задняя панели изготовлены из крашеной стали. Жаропрочное стекло дверец камеры выпечки. Внутренняя подсветка камеры выпечки. Поверхность для выпечки изготовлена из огнеупорного материала (шамот)</p> <p>Технические характеристики:</p> <ul>
<li>натуральный под</li>
<li>температурный режим от 50 °C до 500 °C</li>
<li>внутренний размер камеры: 660х660х140 мм.</li>
<li>габариты: 975x924x413 мм</li>
...
*(blknotrip + **Rule A applied UA** + **Rule B applied UA** + **AUTHORED RU normalized**. `desc UA==RU` **True** 🔴 (originally — после Rule A/B UA-fix будут differ); `nm_ua`==`nm_ru` UA-leak; `nazv_ru` genuine `Печь для пиццы ITPIZZA MS4` → AUTO. **ITPIZZA continuation** (3rd ITPIZZA MS-серия SKU). MS4 = пицца-печь 1-секция на 4 пиццы Ø32 см / 660×660×140 мм / 380 В / 4,7 кВт / 50-500°C / 90 кг / шамот / 975×924×413. **Rule A applied UA**: `32 див` typo → `32 см` (correct UA unit `см` — `див` is non-existent/mistyped form, likely supplier translate-time typo возможно `див` = mistyped Cyrillic `см`). **Rule B applied UA**: `50&deg;С до 500&deg;С` → `50 °C до 500 °C` (entity → Unicode `°`, Cyr `С` → Lat `C`, пробелы added). AUTHORED RU 1:1 mirror UA structure 1 `<p>` + 1 `<p>Технічні</p>` + 1 `<ul>/7li` (multi-line li layout preserved). SOFT применено: identical SKU 70/71 set + (a) `на 4 піци діаметром до 32 см` → `на 4 пиццы диаметром до 32 см` (UA после Rule A); (b) `з високоякісної нержавійючої сталі` → `из высококачественной нержавеющей стали` (UA `нержавійючої` — UA-supplier нестандартная форма vs preferred `неіржавкої`, both → `нержавеющей`); (c) `Бічні і задня` → `Боковые и задняя` (`і` vs SKU 70 `та` — UA-supplier intra-серия conjunction inconsistency LIVE preserve UA, RU normalized `и`); (d) `Поверхня для випічки` → `Поверхность для выпечки` (vs SKU 70/71 `для випікання` — UA-supplier intra-серия variation LIVE preserve UA, RU `выпечки` accordingly); (e) `температурний режим від 50 °C до 500 °C` (UA после Rule B) → `температурный режим от 50 °C до 500 °C`. UA-cell mod b9 += 1 (combined Rule A + Rule B в одном cell). RU-cell mod b9 += 1 (AUTHORED). **POL1/POL2/POL3/POL4 НЕ trigger** (UA fixes via Rule A/B, не POL). ITPIZZA MS4 — **FIRST b9 SKU где Rule A + Rule B применены вместе** в одном cell (Rule A `32 див`→`32 см` + Rule B `50&deg;С до 500&deg;С`→`50 °C до 500 °C`). META always faithful. Открытых вопросов 0.)*

*(scoped к row Артикул=490788756)*
