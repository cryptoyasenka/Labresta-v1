...
**Семейства:** **3 раздела** `Холодильне та морозильне обладнання/...`. **Раздел 1 — `Морозильні шафи`** (SKU 1-14, 14 SKU): Tefcold ×7 (UF600S/UFSC370G/UFSC371SD/UFSC370G Black/NF2500G/NF2500G RAL7024/UFFS370SD) + Gooder ×3 (SF400/SF600/GN-1410BT) + Snaige ×3 (CF27SM-T1000FQ/T1CB0FQ/T1EP0F) + TATRA SKIP-НП (TRC1400BT). **Раздел 2 — `Морозильні столи`** (SKU 15-59, 45 SKU, основной раздел chunk-028): Apach SKIP-НП ×3 (AFM 02 BT/03 BT/04 BT) + TATRA SKIP-НП ×2 (TRC02BT/TRC03BT) + FROSTY ×4 (SNACK 2100BT/3100BT/GN 2100BT/GN 3100BT) + Forcar ×2 (GN3100BT/G-SS45BT) + REEDNEE ×3 (GN2100BT/GN3100BT/SDF901) + HURAKAN SKIP-НП ×2 (HKN-GXFC2GN/HKN-GXFC3GN) + Tecnodom ×2 (TF02MIDBT/TF02MIDBTAL) + Tecnodom TF03MIDBT + Tefcold ×9 (SA910BT/SK6310BT/+SP/SK6310BT/CF7210/CF7310/CF7410/SK6210BT/+SP/SK6410ВТ/+SP/GF72) + COOLEQ ×3 (GN2100BT/GN3100BT/SS45BT) + Brillis ×3 (BGL2-R290/СN2100/СN3100) + Forcar G-SS45BT + Gooder ×3 (GN2100ВТ/GN3100ВТ/GN4100ВТ — full GN-family) + FAGOR SKIP-НП ×3 (MFN-135 EXP/MFN-180 EXP/CCN-2G GR) + GoodFood ×2 (GF-GN2100BT-HC/GF-GN3100BT-HC) + GGM GTS227 + Hendi 233399 Kitchen Line 600 + Forcold M-GN2100BT-FC. **Раздел 3 — `Холодильні шафи для вина`** (SKU 60-61, 2 SKU, ПЕРВЫЙ winefridge раздел в проекте, **SECTION TRANSITION** SKU 59→60): Tefcold TFW400-2F (Danish TFW Frameless 2-glass-door безрамочный winefridge) + GoodFood RT400L2 (floor-standing wine display 400 л — ПЕРВЫЙ GoodFood winefridge в проекте, LAST SKU chunk-028 61/61). Кумулятивно SKIP-НП chunk-028 = **11** (Apach ×3 + TATRA ×3 + Fagor ×3 + HURAKAN ×2). Распределение по батчам: b1=0, b2=3, b3=3, b4=1, b5=1, b6=2, b7=0, b8=1.

**Авто-применено (locked, подтверждения не требует):**
- **🔴 RU=UA full translate + Назв.мод (RU) UA-leak → корректное Название (RU) триплетом (blk):** **15 из 61**. «Описание (RU)» = дословная укр. копия UA (`desc UA==RU` **True**) → самостоятельный RU тег-в-tag 1:1 + Назв.мод (RU) с укр. leak char-level (`Стіл`/`шафа`/`розрахований`/`нержавіюча`-формы и пр.) → корректное русское «Название (RU)» триплетом. Модель-коды language-neutral нетронуты (FROSTY/REEDNEE/Tecnodom/COOLEQ/Forcar/Gooder/Hendi/Forcold). NEW DURABLE PRECEDENTS chunk-028:
  - **ПЕРВЫЙ Hendi blk триплет** (b7 SKU 49 Hendi 233399 Kitchen Line 600) — **NEW SUBTYPE `mixed-bilingual`** (du==dr identical, но body partially translated: intro + first 3 li + last li RU genuine, middle 8 li UA — lesson: для du==dr identical ВСЕГДА char-level full-body UA scan, NOT head-only probe). Hendi supplier convention durable: Lat `x` в dim-values `1800x600x(H)850 мм` preserve (distinct от Gooder Cyr `х` mass-translate Policy B/C); Cyr В `220 В.` space preserve; supplier-defect SOFT normalize `близько 15 кожну полицю`→`около 15 кг на каждую полку` (add unit); `Місткість 390 к.`→`Вместимость 390 л.` (truncation fix); `Хладоген`→`Хладагент` typo correct.
  - **ПЕРВЫЙ Forcold blk триплет** (b8 SKU 57 Forcold M-GN2100BT-FC) — Forcold supplier convention durable: decimal **dot** `0.68 кВт` (distinct от Gooder/Hendi запятая); `220V` Lat no-space (distinct от Hendi `220 В.` Cyr space); `AISI201` Lat no-space (distinct от Gooder/Hendi `AISI 304` пробел); supplier-shorthand `тнар`→`темп. окруж. среды` canonical expand; `холодоагент`→`хладагент` lowercase canonical; `Об&#39;єм`→`Объём` ё drop апостроф.
  - **ТРИ Gooder blk триплет** (b5 SKU 38 GN2100ВТ + b5 SKU 39 GN3100ВТ + b7 SKU 55 GN4100ВТ — **full Gooder GN-family**). Gooder durable: inverse postfix-Cyr+entity `С&deg;` preserve verbatim (NOT `&deg;С` — supplier-specific quirk); Cyr `х` U+0445 → Lat `x` U+0078 в dim header AND value Policy B/C translate; Cyr `ВТ` U+0412+U+0422 в коде модели `GN[N]100ВТ` preserve; collapsed-li three-in-one supplier signature preserve; UA-typo `Климатичний`/`всередені`/`Хлодоген`→`Климатический`/`внутри`/`Хладагент` SOFT normalize.
  - **Tecnodom/COOLEQ/FROSTY/Forcar/REEDNEE blk triплеты** — стандартный авторский RU 1:1, supplier-конвенции совпадают с precedent-ами chunk-026/027.
- **🔴 RU=UA full translate, БЕЗ триплета (blknotrip; Назв.мод (RU) уже чистый русский):** **0 из 61** (НЕ встретился — все blk SKU имели char-level UA-leak в Назв.мод).
- **blknochg (RU-описание самостоятельный готовый перевод ≠ UA verbatim, Назв.мод (RU) чистый — обе локали без изменений):** **27 из 61**. RU≠UA verbatim (`desc UA==RU` False), украинского leak в Назв.мод нет → genuine RU НЕ переписывается (LIVE-магазин Horoshop). Включает Tefcold ×9 (моро-столы CF7210/CF7310/CF7410/SK62/SK63/SK64/SA910BT/GF72), Tefcold TFW400-2F winefridge (SECTION 3), GoodFood ×3 (GF-GN2100/GF-GN3100/RT400L2 winefridge — LAST SKU 61/61), REEDNEE SDF901 (b8 Chinese supplier-template), Brillis ×3, Gooder SF600/GN-1410BT, Snaige ×3, FROSTY SNACK 2100BT/3100BT, Forcar GN3100BT, Tecnodom TF03MIDBT, COOLEQ (НЕ встретился в blknochg — все COOLEQ ушли в blk), GGM GTS227 четырёхдверный.
- **blknochgeq (LIVE Horoshop genuine RU subtype, `desc UA==RU` False, но `nm_ru==nm_ua==nazv_ru`):** **8 из 61** — **ВСЕ b1 SKU 1-8** (Tefcold ×7 — UF600S/UFSC370G/UFSC371SD/UFSC370G Black/NF2500G/NF2500G RAL7024/UFFS370SD + Gooder SF400). LIVE Horoshop genuine RU subtype: supplier-side RU body ≠ UA verbatim, но title (nm_ru) совпадает с UA/nazv — title-level чистый рус без перевода тела, body не переписывается.
- **SKIP-НП (НП-эксклюзивный бренд — RU тело из фида НП позже, ячейки fixed.xlsx не тронуты):** **11 из 61** — **Apach ×3** (SKU 15 AFM 02 BT `527344073` + SKU 16 AFM 03 BT `527347411` + SKU 17 AFM 04 BT `595397992`, b2-b3) + **TATRA ×3** (SKU 14 TRC1400BT `2062029823` + SKU 18 TRC02BT `2062025984` + SKU 19 TRC03BT `2062029113`, b2-b3) + **Fagor ×3** (SKU 40 MFN-135 EXP `2204095479` + SKU 43 MFN-180 EXP `2282947003` + SKU 44 CCN-2G GR `2375839553`, b5-b6) + **HURAKAN ×2** (SKU 26 HKN-GXFC2GN `1082407888` + SKU 59 HKN-GXFC3GN `1082410869`, b4 + b8 — **ПЕРВЫЙ Hurakan в chunk-028**). RU не переписывается, зонды/scratch не делались (forward-only правило SKIP-НП, приоритет над переводом, перекрывает blk-паттерн даже при `desc UA==RU` True); считается отдельной категорией в 61/61.

**Faithful (locked, не тронуто):** напряжение/voltage везде — значение НИКОГДА (метки переведены; per-supplier: Forcold `220V` Lat no-space / Hendi `220 В.` Cyr space / Gooder `220 В` / combo-Lat-V — нетронуто, разные значения = разные модели НЕ десинк); META keywords всегда (genuine META UA!=RU faithful); decimal per-supplier: Forcold dot `0.68`, Gooder/Hendi запятая `10,89` — preserve verbatim, НЕ unify; AISI per-supplier: Forcold `AISI201` no-space / Gooder-Hendi `AISI 304` space — preserve verbatim; Cyrillic х/Х U+0445 vs latin x U+0078 в габаритах — supplier-specific Policy B/C, preserve verbatim или Cyr→Lat mass-translate per-Gooder rule; literal ° → `&deg;` (UA-источник); UA-апостроф `&#39;`/U+0027/`&rsquo;` в authored RU дроп; `&deg;`/`&ndash;`/`&laquo;`/`&raquo;`-энтити и literal U+2014 `—` источника зеркалятся 1:1; `R290`/`GN 1/1` хладагент/стандарт Lat verbatim; supplier shorthand `тнар`→`темп. окруж. среды` canonical expand (Forcold); `Хладоген`/`Хлодоген`/`Холодоагент`→`Хладагент` typo SOFT normalize (Hendi/Gooder); `Климатичний`/`всередені`→`Климатический`/`внутри` SOFT (Gooder UA-typo) — НЕ нумеруются, рассогласование источника. blknochgeq LIVE Horoshop genuine RU (b1 SKU 1-8) + blknochg LIVE Horoshop per-SKU format preserve verbatim, НЕ unify.

**🔶 Открытые числовые/модельные расхождения источника: 0.** chunk-028 — **0 Открытых вопросов** (clean close — за все 8 батчей не зафиксировано ни одного нумерованного OQ; все SOFT-нормализации supplier-side faithful, не требуют решения Yana).

Прочее: 🔴 blk 15 = авто-перевод укр. копии (Назв.мод-leak триплет НЕ customer-facing, модель-коды FROSTY/REEDNEE/Tecnodom/COOLEQ/Forcar/Gooder/Hendi/Forcold language-neutral); blknotrip 0 (не встретился — все blk имели char-level UA-leak в Назв.мод); blknochg 27 RU genuine не переписывается LIVE (Tefcold ×9 моро-столы + Tefcold TFW400-2F winefridge + GoodFood ×3 + REEDNEE SDF901 + Brillis ×3 + Gooder SF600/GN-1410BT + Snaige ×3 + FROSTY SNACK ×2 + Forcar GN3100BT + Tecnodom TF03MIDBT + GGM GTS227); blknochgeq 8 LIVE Horoshop genuine RU subtype (b1 SKU 1-8 Tefcold ×7 + Gooder SF400); SKIP-НП 11 (Apach ×3 + TATRA ×3 + Fagor ×3 + HURAKAN ×2, тело из фида НП позже); Cyrillic-х/множители идентичны обеим локалям → НЕ нумеруются (precedent ch017-027); voltage разные значения = разные модели НЕ десинк; SOFT (faithful, не нумеровано): `Обсяг`/`Об'єм`/`об&#39;єм`→`Объём`/`объём` ё drop апостроф, `Хлодоген`/`Хладоген`/`холодоагент`→`Хладагент`/`хладагент`, `Климатичний`/`всередені`→`Климатический`/`внутри` (Gooder UA-typo), supplier shorthand `тнар`→`темп. окруж. среды` (Forcold canonical expand) — рассогласование источника, НЕ перевод-десинк.

*(scoped: chunk-028 = 61 строка по `Артикул`; следующий — chunk-029; chunk-001 SKIP навсегда; chunk-≤027 закрыты)*
