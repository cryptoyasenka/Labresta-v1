...
**Result: 4 promoted to CONFIDENT (analogy agrees), 3 deferred (0 products this import), 3 then deepened with the actual SKUs (2026-05-31) → #13 + #48 SKU-corroborated (recommend CONFIRM), #42 isolated as the one genuine open call (recommend `null` / per-SKU). No JSON value changed** — #42 still force-maps in the JSON as drafted; null it (or split per-SKU) for the safe behavior. The measured feed-tier 99.4% includes #42's 6 force-mapped rows (the soft spot); per-row evidence in the "Still flagged" bullets below.
...
- **#42 Вітрини вертикальні холодильні — the one genuine open call → recommend `null` or per-SKU.** The feed category is **heterogeneous**: COLD SW 504 is a *confectionery* display, SCAN RT 82 BE is *tabletop*, RT 82 WE is *demo*, RTC 237 is an upright *cabinet*, and 10× HURAKAN HKN-UPD* are upright glass display cases. A single map to `…/Гірки-регали пристінні` (wall multideck) would **mis-shelve most of them** — which is exactly why analogy split (4/6 кондитерські, 2/6 настільні). By this draft's own null-rule ("no single store label fits"), #42 is best left **`null`** (→ per-product analogy) or mapped **per-SKU**. **The JSON still force-maps #42 to Гірки-регали** — if you upload it as-is those rows go there; null it (or split per-SKU) for the safe behavior.
