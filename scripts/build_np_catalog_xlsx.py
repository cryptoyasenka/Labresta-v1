...
NP-exclusive SKU, emitting one native [КАТАЛОГ]-schema workbook that Horoshop
auto-maps on import (FINAL-MODEL.md §1, §3). Yana imports it by hand
(invariant #13) — this script only builds the file, never touches the store.
...
the import preview — see FINAL-MODEL.md §3). Default output: instance/.

Read-only against the DB except for the optional file write. Dry-run-safe.
"""
...
def run(feed_path: str, out_path: str | None) -> None:
    ...
    with app.app_context():
        ...
        if out_path is None:
            ...
            out_path = str(
                Path(app.instance_path) / f"np-catalog-{ts}.xlsx"
            )
