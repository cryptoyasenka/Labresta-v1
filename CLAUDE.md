...
Flask-приложение, синхронизирует цены и наличие поставщиков (MARESTO и др.) с интернет-магазином **Horoshop** (labresta.com.ua). Приложение генерирует узкие YML-фиды (`labresta-prices.yml`, `labresta-availability.yml`) и отдаёт их Horoshop на импорт. **Это live-магазин**, любые ошибки матчинга = реальные убытки.
