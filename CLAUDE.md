...
### 15. Bracket-token containment — двухступенчатый gate
Скобки в названиях разделяют на три класса: voltage/quantity noise (отбрасывается), descriptive specs (тоже noise, но через расширенный regex), и **variant discriminator** (markers типа «no stand», «тефлон», «робоча станція», «з підставкою»). Discriminator — то, что меняет SKU.

**Два независимых гейта работают подряд:**
- **Step 4.85** (`app/services/matcher.py:1265+`) — симметричный: обе стороны имеют paren-discriminator, пересечение пустое → reject. Пример: SP `(no stand)` vs PP `(на базі)` — `{stand}` ∩ `{базі}` = ∅.
- **Step 4.87** (`app/services/matcher.py:1293+`) — асимметричный: одна сторона имеет discriminator, у другой этих токенов нет нигде в name+article+display_article → reject. Пример: SP `(тефлон)` vs PP `Sirman Mantegna 300` (нет «тефлон»).

**Bypass-условия Step 4.87:**
1. `_skip_post_gates` (display_article fast-match) — manufacturer SKU equality сильнее.
2. `supplier_article` (≥4 chars) verbatim substring в PP полном тексте — paren PP это specs, не variant.
3. `_has_base_package_marker` — `(no ...)` / `(без ...)` это base-version, не feature; матчится в plain катальную базу.

**Noise-расширения** (`app/services/matcher.py:322`, `_PAREN_NOISE_RE`):
- `\d+\s*[вbу]` без word boundary — ловит glued voltage (`380в`, `220b`).
- Descriptive слова: `рівн\w*|уровн\w*|колб\w*|персон\w*|автомат\w*|ручн\w*|технолог\w*|приготуванн\w*`.
- Prepositions UA/RU: `на|за|до|для|з|зі|по`.

- Helper: `_has_base_package_marker` (`app/services/matcher.py:352`) — определяет `\bno\b|\bбез\b` в скобках.
- Тесты: `tests/test_matcher_bracket_discriminator.py` (Step 4.85, 7 тестов), `tests/test_matcher_bracket_containment.py` (Step 4.87, 12 тестов).

**Что нельзя ломать**: при добавлении новых noise-слов проверять что `(no stand)` всё ещё даёт `_has_base_package_marker=True` (если съесть `no` — base-package detection сломается); и что Step 4.85 reject case `(no stand)` vs `(на базі)` остаётся (`stand` 5 chars не должен попасть в noise). Тесты обоих файлов — fence.
